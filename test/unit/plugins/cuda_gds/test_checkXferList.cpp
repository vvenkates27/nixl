...
#define TEST_PHRASE "NIXL checkXferList Test Pattern"
...
void print_usage(const char* program_name) {
    std::cerr << "Usage: " << program_name << " [options] <directory_path>\n"
              ...
              << "\nExample:\n"
              << "  " << program_name << " -n 5 -s 1048576 /tmp\n";
}
...
bool run_single_batch_test(const std::string& dir_path, int num_entries, size_t transfer_size) {
    std::cout << "\n=== Test 1: Single batch with all entries succeeding ===" << std::endl;
    ...
    // Initialize agent
    nixlAgent agent("TestCheckXferList", cfg);

    // Configure GDS backend
    params["batch_pool_size"] = "8";
    params["batch_limit"] = "128";

    ret = agent.createBackend("GDS", params, gds);
    if (ret != NIXL_SUCCESS || gds == NULL) {
        std::cerr << "Failed to create GDS backend" << std::endl;
        goto cleanup;
    }
    ...
    for (int i = 0; i < num_entries; i++) {
        ...
        // Create test file
        std::string filename = dir_path + "/" + generate_test_filename("test_single", i);
    ...
    {
        nixl_xfer_dlist_t vram_list = vram_for_gds.trim();
        nixl_xfer_dlist_t file_list = file_for_gds.trim();
        nixlXferReqH* write_req = nullptr;

        ret = agent.createXferReq(NIXL_WRITE, vram_list, file_list, "TestCheckXferList", write_req);
        if (ret != NIXL_SUCCESS) {
        ...
        std::cout << "Test 1 PASSED: All " << num_entries << " entries completed successfully" << std::endl;
    }
...
bool run_in_progress_test(const std::string& dir_path, int num_entries, size_t transfer_size) {
    std::cout << "\n=== Test 2: In-progress status tracking ===" << std::endl;
    ...
    nixlAgent agent("TestInProgress", cfg);
    ...
    for (int i = 0; i < num_entries; i++) {
        ...
        std::string filename = dir_path + "/" + generate_test_filename("test_progress", i);
        fd[i] = open(filename.c_str(), O_RDWR|O_CREAT, 0744);
        if (fd[i] < 0) {
            std::cerr << "Failed to open file: " << filename << std::endl;
            goto cleanup;
...

    {
        ...
        ret = agent.createXferReq(NIXL_WRITE, vram_list, file_list, "TestInProgress", write_req);
        if (ret != NIXL_SUCCESS) goto cleanup;
        ...
        // Poll for status and track progress
        std::vector<nixl_status_t> entry_status;
        ...
        while (status == NIXL_IN_PROG && poll_count < 100) {
            ...
            // Check if we see any in-progress entries
        ...
        std::cout << "Test 2 PASSED: Status tracking worked correctly (polled "
                  << poll_count << " times)" << std::endl;
    }
...
int main(int argc, char *argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "n:s:h", long_options, NULL)) != -1) {
        switch (opt) {
            case 'n':
    ...
    std::cout << "\n============================================================" << std::endl;
    std::cout << "           NIXL GDS checkXferList Unit Tests               " << std::endl;
    std::cout << "============================================================" << std::endl;
    ...
    bool test1_passed = run_single_batch_test(dir_path, num_entries, transfer_size);
    bool test2_passed = run_in_progress_test(dir_path, num_entries, transfer_size);

    std::cout << "\n============================================================" << std::endl;
    std::cout << "                    TEST SUMMARY                            " << std::endl;
    std::cout << "============================================================" << std::endl;
    std::cout << "Test 1 (Single batch success): " << (test1_passed ? "PASSED" : "FAILED") << std::endl;
    std::cout << "Test 2 (In-progress tracking): " << (test2_passed ? "PASSED" : "FAILED") << std::endl;
    std::cout << "============================================================" << std::endl;
